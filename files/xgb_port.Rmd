...
offset_date <- 0                                    # 0 => 5 years
max_depth <- 5                                      # Maximum nb of levels for the trees
q <- 0.1                                            # Filtering intensity

ptm <- proc.time()                                  # Initiate CPU time
for(t in 1:Tt){
    if(t%%12==0){print(dates_oos[t])}
    x <- filter(data_mat_rk, DataDate < dates_oos[t], DataDate > dates[t + offset_date])  # Rolling window
    # The line below determines the filtering intensity. Omit it to remove the filter.
    x <- filter(x, R12M_F >= quantile(x$R12M_F,1-q) | R12M_F<=quantile(x$R12M_F,q))       # Filtered data
    ...
    fit <- xgb.train(eta = 0.7,                    # Learning rate
