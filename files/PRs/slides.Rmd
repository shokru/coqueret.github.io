...
date: .font90[**EMLYON** Presentation] 2020-10-19
output:
...
Once $(a,b)$ has been estimated, the natural prediction for $y_{t+k}$ is $\tilde{y}_{t+k} = \hat{a}+\hat{b}x_t$ and the usual way to measure its **accuracy** is to compute the (out-of-sample) Mean Squared Error (MSE), or quadratic loss:
...
If all parameters are **known**, then the loss is minimized by the **optimal slope**:
...
These hypotheses are rather common in the literature.
...
- .bluep[**Valkanov JFE 2003**]: long horizon estimates are skewed by construction (1/2)   
- .bluep[**Amihud & Hurvich JFQA 2004**]: extension to multivariate regressions (1/2)    
- .bluep[**Campbell & Yogo JFE 2006**]: efficient tests for inference (1/3)   
- .bluep[**Boudoukh et al. RFS 2008**]: long horizon estimates are skewed by construction (2/2)     
- .bluep[**Bandi et al. J. Econom 2019**]: predictability attains its peak around 12-15Y   
- .bluep[**Jonhson RAPS 2019**]: efficient tests for inference (2/3)  
- .bluep[**Andersen & Varneskov J. Econom 2020**]: extension to multivariate regressions (2/2)  
- .bluep[**Xu RFS 2020**]: efficient tests for inference (3/3)  
]
]
...
.font120[**When is using the OLS estimator in predictive regressions a good idea?**] (i.e. when $R^2>0$)

The (complicated) answer depends on the **model parameters**:
...
The loss (mean squared error) has the following properties:   

- it does not depend on the constants of the processes $\alpha_x$ & $\alpha_y$;   
- it does not depend on the variance of innovations of $x$, $\sigma_x^2$;      
- it is **proportional** to the variance of innovations of $y$, $\sigma_y^2$;    
- it is **symmetric** in $\rho$, the correlation between the two innovation processes.

Moreover, if means are known, $\rho=0$ and $\rho_x=\rho_y$, then we can show that short term forecasting is "easier", i.e., that
...
---
# Sensitivity: short term horizon ( $R^2$ version)
...
.font70[Big change for **high persistence** in *y* (right column, $L_o$ not the best!).   
...
---
# Sensitivity: long term horizon ( $R^2$ version)
