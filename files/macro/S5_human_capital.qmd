...
wb_raw |>
  ...
  theme_classic() + ylab("Average GDP per capita") + xlab("Average human capital index") +
...
wb_raw |>
  ...
  theme_classic() + ylab("Average GDP growth rate") + xlab("Average human capital index") +
  geom_text_repel(aes(label = country))
```

Things are much less clear for growth...
...
If we assume **constant returns to scale**, $\alpha+\beta < 1$ because else $\gamma=0$ (and labor and productivity are out of the game). And obviously, $\beta > 0$ for the same reason (human capital), hence this imposes $\gamma_k=0$. 

::: {.callout-important collapse="false"}
Again, this means **zero growth in equilibrium**!   
:::

We are not really more advanced - but this was to be expected.
...
and $K/H$ does not have a simple expression (algebraic equation). 
...
$$y=Ak,$$
hence its name. As in the **Ramsey model**, if we assume $n=0$, capital evolves as:
...
To generate growth, one way may be to rely on some form **productivity**. From a **human capital** perspective, the idea here is to assume that **experience** gets workers better at **production**. Below, we follow the survey [*Learning by doing*](https://digitalcommons.fiu.edu/economics_wps/42/), Section 5. 
...
If we assume $\frac{\dot{k}}{k}=0$, then growth comes from learning only. We can find a situation where all growths are positive. It corresponds to $\frac{\dot{y}}{y}=\frac{\dot{k}}{k}$ and $\frac{\dot{A}}{A}=(1-a)\frac{\dot{k}}{k}$. This yields
$$\frac{\dot{k}}{k}=\frac{\dot{y}}{y}=\frac{\lambda n}{1-a-\lambda}$$
Note that $n=0$ implies $a+\lambda=1$ (constant returns to scale). If population grows, then it must hold that $a+\lambda<1$ (DRS). 
