...
where $b$ is some constant that comes from country specific welfare programs. $\beta=(1+\rho)^{-1}\in(0,1)$ is the **discounting factor**.  
...
Hence: there is no big change, except that (a higher) $\lambda$ **reduces** the reservation wage! Indeed, if there is more **layoff uncertainty**, it is logical that the job seeker will accept a slightly **lower salary**. 
...
Why not try a calibration exercise?
...
$$w = y-(r+\lambda)(1-\theta)S,$$ {#eq-w6}
with: $\theta$ the bargaining power of the worker, $y$ her productivity, $r$ is a discounting rate (for time preference), $\lambda$ the job loss rate and $S$ the surplus (@eq-S). 
...
In the end (See Section 4.4 of [Rogerson et al.](https://www.aeaweb.org/articles?id=10.1257/002205105775362014)), we obtain:

$$w(y) = b + \frac{\alpha_U \theta k}{\alpha_V (1-\theta)} +\theta \left(y -  b - \frac{\alpha_U \theta k}{\alpha_V (1-\theta)} \right)$$
The wage is clearly associated with the productivity of the worker. If the worker has more bargaining power (higher $\theta$), then, naturally her wage increases with $y$. Here, the cost of recruiting $k$ increases the wage.


## An attempt at calibration

Can we ground these formulae into real-world data? Let's consider @eq-w6 applied to US.

- $w$: (target) **wage** - World Bank or [ILO Data](https://rshiny.ilo.org/dataexplorer39/?lang=en&segment=indicator&id=EAR_4HRL_SEX_ECO_CUR_NB_A): roughly 40$ per hour on average;      
- $y$: **labor productivity** - [ILO Data](https://ilostat.ilo.org/topics/labour-productivity/): roughly 80$ per hour;
...
This implies $S \approx 200,000\$$, which is very high. We recall that 
$$S=J-V+W-U,$$
