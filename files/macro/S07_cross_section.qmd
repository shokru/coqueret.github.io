...
$$Y=\prod_{n=1}^N X_n^{a_n}, \quad X_n >0$$
so that we envision $N$ different **factors** contributing to the economic output. Taking the logarithm:
...
Let's have a look at missing data, **after imputation**!. In some cases, when no data exists, forward-filling is not possible.
...
vars <- c("gdp_percap", "savings_rate", "inflation", "trade", "debt", "capital_formation" , 
          "pop_growth", "RD_percap", "educ_level", "educ_spending")
...
Many large and developed countries do not make it to the top, mostly due to the R&D, but also debt variables.    
...
So here, it seems we are relatively fine.



# Panel models

We now turn to an exploration of the concepts and variables seen and mentioned until today. Indeed, models are only worthwhile if they are able to explain and predict salient empirical properties of the economy.

::: {.callout-note collapse="false"}
## Note 

We follow here a panel approach from [Economic Growth in a Cross Section of Countries](https://academic.oup.com/qje/article-abstract/106/2/407/1905452), though we apply it to GDP percapita and not to *raw* GDP:
...
As a first attempt, we proceed with the traditional two-ways fixed effect model, meaning that the equation is:
...
The `{plm}` package is likely the most used for panel models in R. We use it below. 
...
#| class-source: my_class
#| classes: my_class
plm_data <- wb_growth |> dplyr::select(all_of(c("country", "year", "gdp_growth", vars))) |> 
...
Here, the **savings rate**, **capital formation** both have significant positive coefficients... **education** and **population growth** have *negative* coefficients.   
...
#| class-source: my_class
#| classes: my_class
fml_0 <- "gdp_growth ~ gdp_percap + savings_rate + inflation + trade + debt + "
fml_0 <- paste(fml_0, "capital_formation + pop_growth + RD_percap + educ_level + educ_spending")
...
But there are a few conflicts, too: **trade**, **RD_percap** and **education**. One subtlety: for **GDP per capita**, signs remain the same, but the significance changes drastically, passing above usual threshold when only time fixed effects are included. The same thing happens for **capital formation**, though here it it the TWFE model that rejects the null.

These results suggest that **population growth** and the **savings rate** are the only solid drivers of growth in wealth. 

## Grouping

Until now, we have run the models on *all* of the data, but we could also consider sub-groups, either geographical ones, or income clusters. We could use grouping below, but in fact, for lower income countries, **we do not have enough data points** to proceed with estimation...
...
Pretty consistent! Except perhaps for one variable (education)...
...
Hard to make a clear case...   

- the 2000-2015 period speaks in favor of convergence;     
- but since then, things have reverted;    
- the most recent points (2024) indicate a status-quo. 
