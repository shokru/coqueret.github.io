...
plm_data <- wb_growth |> dplyr::select(all_of(c("country", "year", "gdp_growth", vars))) |> 
  na.omit() |> group_by(country) |> mutate(n = n()) |> filter(n > 21) |> select(-n)
...
These results suggest that **population growth** and the **savings rate** (to a slightly lower extent) are solid drivers of growth in wealth. 
...
data_energy |> 
  ...
  ggplot(aes(x = year, y = value, fill = var)) + geom_area() +
  theme_classic() + 
...
Let us also look at **correlations**...
...
cor(data_energy_wide |> select(coalcons, gascons, oilcons, renewables),
    use = "pairwise.complete")
```

Basically, we should only keep renewables and gas...
But let's continues anyway out of curiosity.  
...
#| classes: my_class
#| column: margin
data_join <- wb_growth |> select(-region) |>
  full_join(data_energy_wide, by = c("year", "country"))
data_join |> select(-region, -income, -subregion) |>  na.omit() |> head()
```
...
#| class-source: my_class
#| classes: my_class
fml_0 <- "gdp_growth ~ gdp_percap + savings_rate + pop_growth + RD_percap + educ_level + oilcons + gascons + coalcons + renewables"
fml <- as.formula(paste(fml_0, "| year + country"))
feols(fml, data = data_join) |> summary()
```

The negative coefficient on oilcons is troublesome.   
