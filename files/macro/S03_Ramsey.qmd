...
This comes from $\frac{\partial Y}{\partial K}=\beta (1-1/\epsilon)\frac{\epsilon}{(\epsilon-1)} K^{-1/\epsilon} \left(\beta K^{1-1/\epsilon} + \alpha (AL)^{1-1/\epsilon} \right)^{1/(\epsilon-1)}$ and 
...
Hence, noticing $Y^{-1+1/\epsilon}= \left(\beta K^{1-1/\epsilon} + \alpha (AL)^{1-1/\epsilon} \right)^{-1}$,
upon equilibrium, the labor share is 
...
The solution of the [matrix differential equation](https://en.wikipedia.org/wiki/Matrix_differential_equation) takes the form
$$\hat{k}_t=\hat{k}^* + \psi_1 e^{\beta_1 t} + \psi_2 e^{\beta_2t}, $$

where the $\beta_i$ are the **eigenvalues** of the transition matrix above and the $\psi_i$ are some constants. This is in fact an approximation: it should depend on the eignevectors too - but we omit this for now.


In the usual parameter configuration, the eigenvalues have opposite signs. Because we are interested in equilibria, the system must not blow up, hence the $\psi_i$ associated with the positive eigenvalue must be zero. To identify the remaining constant, it suffices to take $t=0$. In the end, the dynamics of the log of capital per capita is
...
Omega <- matrix(c(rho - n, -1, 
                  -sigma*c_star*A*alpha*(1-alpha)*k_star^(alpha-2), 0), 
                nrow = 2, byrow = T)
Omega
```
...
x <- path_generator(n_steps, A, alpha, rho, k_0, c_0, dt)
```
