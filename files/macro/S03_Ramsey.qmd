...
Perhaps one of the most important changes compared to the Solow model is the handling of **consumption** (which will later on be studied in greater depth). In the **Solow model**, consumption was inferred from a savings rate, $s$. In the **Ramsey model**, as is standard in mainstream economics, the (representative) consumer has a total utility of:
...
If we assume **zero depreciation** ($\delta=0$), the motion of capital is then given by $\dot{K}_t=Y_t-C_t$ (by constraint). Thus, in per-capita terms,
...
and the first order conditions (FOCs) with respect to $x$ and $\lambda$ will impose restrictions on the solution - often leading to a unique value. Note that $x$ (and $\lambda$) may be of vector type.
...
which is the dynamic equivalent of the Lagrangian in static optimization - modulo the fact that the multiplier is associated with $\dot{k}_t$ (and not the exact budget constraint). $\lambda_t$ is the co-state variable, the counterpart of the Lagragian multiplier. The first order conditions (FOCs) are then, for the control variable:
...
::: {.callout-note collapse="false"}
## Derivation of the co-state equation
...
The dynamics of the models are given by @eq-budget for **capital** and @eq-euler for **consumption**. Two equations and two unknowns. 
...
$$\frac{wL}{Y}=\alpha \left( \frac{AL}{Y}\right)^{1-1/\epsilon}$$
