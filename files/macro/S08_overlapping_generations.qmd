...
which simplifies to
$$\frac{c_{t,\text{work}}}{c_{t+1, \text{retire}}}=\left(\frac{1+r_{t+1}}{1+\rho} \right)^{\sigma}.$$
This can be viewed as the **Euler equation** for the generation "*born*" at time $t$.
...
With reasonable values, what kind of rate does this imply?
...
For simplicity, we assume only two periods.
...
$$b_t=(1+n)d_t$$ {#eq-payg}
The $(1+n)$ term reflect the potential **difference in population sizes**. If the population grows ($n>0$), then there are more active workers than retirees and the latter can comfortably benefit from a larger benefit. Otherwise (where many developed countries are headed), it's more complicated!
...
which is lower than @eq-PAYG when $r>n$. Hence this seems like a pleasant solution! 
