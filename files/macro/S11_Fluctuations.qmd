...
For productivity, the trend is exactly that from the data (makes sense), but for leisure, it's hard to tell.   
Maybe the model serves as a tool to determine reasonable values for $l$!
...
Total **output (Y)** equals **total planned spending** (aggregate demand):
...
#### LM Equation and meaning

$$\frac{M}{P} = L(i, Y)$$

Equilibrium in the **money market** occurs when:

- $M/P$: *real money supply* (nominal supply set by the central bank, adjusted for prices)  
- $L(i, Y)$: *money demand*, which is a function that: 
...
### 🧭 Policy Intuition (optional extension)
...
We use a simplified linear form for illustration:  

- IS: $i = a - b \times Y $  
- LM: $i = c + d \times Y$

First, the parameters and the generation of data.
...
What happens when there is a policy change?
...
It was introduced by economist **John B. Taylor (1993)** as a way to describe and formalize the behavior of monetary policy under an **inflation-targeting framework**.
...
Where:  
- $i_t$: **nominal policy interest rate** (e.g., the federal funds rate)  
...
> adjusting interest rates to stabilize both **inflation** and **output** around their desired levels.
