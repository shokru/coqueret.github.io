...
We need to subtract the red bars from the green ones and recover the black line... not too far! 

# The Ramsey model with debt/bond
## Formulation

For simplicity, we fix $n=0$ and $L_0=1$ so that population remains constant and all terms can be viewed as *per capita*. As in the **Ramsey model**, the utility is 
...
The first order conditions are then (assuming $\lambda_t\neq 0$)
...
Differentiating the first equation w.r.t. time and plugging @eq-cond_c, we get
...
Furthermore, note that, multiplying @eq-a_dot by $e^{-rt}$ and integrating, we get 
$$a_te^{-rt}-a_0 = \frac{1-e^{-rt}}{r}(f'(k^*)-rk^*-c^*),$$

so that, as $t\rightarrow \infty$ and under non-explosive assumptions, we get an expression for optimal **consumption**:
$$c^* = ra_0+f'(k^*)-rk^*.$$

## Some comments on the model

The most salient property of the model is @eq-const_c. At the optimum (equilibrium), any deviation from $k^*$ or $c^*$ is immediately corrected. The trick is that the residents of the home country are able to adjust by purchasing any amount of required capital - thanks to the bond. This does not affect the national assets, because $a=k+b$, hence reducing $b$ by the same amount that $k$ increases leaves $a$ unchanged. 



# Sustainability of public debt

We leave growth aside for now to focus on the law of motion of **public debt** (from [François Geerolf's lecture notes](https://fgeerolf.com/econ102/public-debt.html)). We write:   
...
wb_raw |>
  ...
  select(country, year, interest_rate, gdp_growth) |>
